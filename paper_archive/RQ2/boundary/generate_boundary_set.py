...
from paper_archive.RQ2.boundary.boundaries import sample_boundary

CHUNK_SIZE = 100_000  # ~4.4MB
SHARD_SIZE = CHUNK_SIZE * 1000  # ~4.4GB

parser = argparse.ArgumentParser()
parser.add_argument("--set", type=str, default="test", help="Set type")
parser.add_argument("--dof", type=int, default=7, help="Degrees of freedom")
parser.add_argument("--num_robots", type=int, default=100, help="Number of robots to generate")
...
SAFE_FOLDER = Path(__file__).parent.parent.parent.parent / 'data' / 'boundary' / args.set
...
file_offset = 0
buffer = torch.zeros(min(args.num_robots * args.num_geodesic * args.num_geodesic_samples, SHARD_SIZE), 11, dtype=torch.float32)
buffer_offset = 0
...
for idx, morph in enumerate(tqdm(morphs, desc=desc)):
    poses, labels = sample_boundary(morph, args.num_geodesic, args.num_geodesic_samples)
